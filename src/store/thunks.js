...
export const fetchCampusStudentsThunk = (id) => async (dispatch) => {
  try {
    let res = await axios.get(`/api/campuses/${id}`);
    dispatch(ac.fetchCampusStudents(res.data.students));
  } catch(err) {
    console.error(err);
  }
}
...
export const deleteStudentThunk = studentId => async dispatch => {
  try {
    let res = await axios.delete(`/api/students/${studentId}`);
    //delete succesful so change state with dispatch
